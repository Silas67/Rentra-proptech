...
const AgentDashboard = () => {
  ...
  const pendingBookings = mockBookings.filter((b) => b.agentId === "u2" && b.status === "pending");

  const copyLink = (code: string) => {
    navigator.clipboard.writeText(`${window.location.origin}/search?ref=${code}`);
    ...
    <div className="min-h-screen py-6">
      <div className="container">
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-3">
          {[
            { icon: MousePointer, label: "Total Clicks", value: totalClicks, color: "text-primary" },
            { icon: Eye, label: "Inspections", value: totalInspections, color: "text-secondary" },
            { icon: DollarSign, label: "Commission Earned", value: `₦${totalCommission.toLocaleString()}`, color: "text-success" },
          ].map((stat) => (
        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {referrals.map((ref) => {
              const property = mockProperties.find((p) => p.id === ref.propertyId);
              return (
                <div key={ref.id} className="flex flex-col gap-3 rounded-xl border bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="font-medium">{property?.title}</p>
                    <p className="text-sm text-muted-foreground">{ref.referralCode} · {ref.clicks} clicks · {ref.inspections} inspections</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant={ref.status === "converted" ? "default" : "secondary"}>{ref.status}</Badge>
                    <Button variant="outline" size="sm" onClick={() => copyLink(ref.referralCode)}>
              ...
              {pendingBookings.map((b) => {
                ...
                return (
                  <div key={b.id} className="rounded-xl border bg-card p-4">
                    <p className="font-medium">{property?.title}</p>
                    <p className="text-sm text-muted-foreground">{b.tenantName} · {b.date} at {b.time}</p>
